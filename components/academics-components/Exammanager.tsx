...
interface ExamFormProps {
  initialValue?: Exam | null;
  onSubmit: (data: Omit<Exam, "id">) => void;
  onCancel: () => void;
}

export function ExamForm({ initialValue, onSubmit, onCancel }: ExamFormProps) {
  ...
  const [examDate, setExamDate] = useState(initialValue?.examDate ?? "");
  const [status, setStatus] = useState(initialValue?.status ?? "Scheduled");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !examDate) return;
    onSubmit({ name: name.trim(), subject, className, examDate, status });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-slate-500">Exam Name</label>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Mid-Term Examination" required autoFocus className={fieldClass} />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-slate-500">Exam Date</label>
          <input type="date" value={examDate} onChange={(e) => setExamDate(e.target.value)} required className={fieldClass} />
        </div>
        <div className="flex flex-col gap-1">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button type="submit" className="px-5 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition-colors">
          {initialValue ? "Save Changes" : "Add Exam"}
        </button>
      </div>
    </form>
  );
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-left font-medium text-slate-500 px-5 py-4">Exam</th>
          ...
          <tbody>
            {exams.map((ex) => (
              <tr key={ex.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50">
                ...
                <td className="px-2 py-4 text-slate-600">{ex.subject}</td>
                <td className="px-2 py-4 text-slate-600">{ex.className}</td>
                <td className="px-2 py-4 text-slate-500">{formatDate(ex.examDate)}</td>
                <td className="px-2 py-4"><StatusPill status={ex.status} /></td>
                <td className="px-5 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button type="button" onClick={() => onEdit(ex)} aria-label={`Edit ${ex.name}`} className="w-8 h-8 flex items-center justify-center rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button type="button" onClick={() => onDelete(ex.id)} aria-label={`Delete ${ex.name}`} className="w-8 h-8 flex items-center justify-center rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors">
