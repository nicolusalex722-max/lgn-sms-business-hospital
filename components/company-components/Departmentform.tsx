...
import type { Department } from "./Departmenttable";
...
export default function DepartmentForm({ initialValue, onSubmit, onCancel }: DepartmentFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ name: name.trim(), code: code.trim().toUpperCase(), description: description.trim(), status });
  };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          rows={3}
          className="px-3 py-2 rounded-lg border border-slate-300 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
        />
      </div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        >
          {initialValue ? "Save Changes" : "Add Department"}
        </button>
      </div>
    </form>
  );
